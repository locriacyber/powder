...
struct tile_info
{
    u8		blockid;		// Block address of this tileset.
};
...
struct map_info
{
    u8		blockid;		// Block address of this map.
    bool	largemap;
};
...
tile_info_ptr ham_InitTileEmptySet(u16 entries, u8 , u8);
void ham_InitBg(int layer, int foo, int bar, int baz);
void ham_CreateWin(int wid, int x1, int y1, int x2, int y2, int inside, int outside, int fx);
